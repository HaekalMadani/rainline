...
class DriverService:
    ...
    def get_all_drivers(sort_by: str = "driver_code") -> List[DriverSummary]:
        ...
        valid_sorts = ["driver_code", "full_name", "average_position", "total_seasons"]
        ...
        with get_db() as conn:
            cursor = conn.cursor()
            cursor.execute(f"""
                SELECT driver_code, full_name, average_position, total_seasons, driver_number, current_team
                FROM drivers
                ORDER BY {sort_by}
            """)
